...
def publish_node_birth():
    ...
    add_metric(payload, "Node Metric0", None, MetricDataType.String, "hello node")
    add_metric(payload, "Node Metric1", None, MetricDataType.Boolean, True)
    add_null_metric(payload, "Node Metric3", None, MetricDataType.Int32)
    ...
    dataset = init_dataset_metric(payload, "DataSet", None, columns, types)
    ...
    metric = add_metric(payload, "Node Metric2", None, MetricDataType.Int16, 13)
    ...
    template = init_template_metric(payload, "_types_/Custom_Motor", None, None)
    ...
    add_metric(template, "RPMs", None, MetricDataType.Int32, 0)
    add_metric(template, "AMPs", None, MetricDataType.Int32, 0)
...
def publish_device_birth():
    ...
    add_metric(
        payload, "input/Device Metric0", None, MetricDataType.String, "hello device"
    )
    add_metric(payload, "input/Device Metric1", None, MetricDataType.Boolean, True)
    add_metric(payload, "output/Device Metric2", None, MetricDataType.Int16, 16)
    add_metric(payload, "output/Device Metric3", None, MetricDataType.Boolean, True)
    add_metric(
        payload,
        "DateTime Metric",
        None,
    ...
    # Create the UDT definition value which includes two UDT members and a single parameter and add it to the payload
    template = init_template_metric(payload, "My_Custom_Motor", None, "Custom_Motor")
    template_parameter = template.parameters.add()
    template_parameter.name = "Index"
    template_parameter.type = ParameterDataType.String
    template_parameter.string_value = "1"
    add_metric(template, "RPMs", None, MetricDataType.Int32, 123)
    add_metric(template, "AMPs", None, MetricDataType.Int32, 456)
...
while True:
    ...
    metric = add_metric(
        payload, None, None, MetricDataType.Boolean, random.choice([True, False])
    )
